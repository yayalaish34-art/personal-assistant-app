// English is the source of truth. Every other dictionary is checked against
// this one by the compiler: `Dict` is keyed by `keyof typeof en`, so a key
// that is not here at all fails to build, while a key that is here but
// missing from a translation is legal and falls back to the English string
// at runtime. That is what makes an unfinished translation safe to ship.
export const en = {
  // Tabs
  'tab.home': 'Home',
  'tab.calendar': 'Calendar',
  'tab.tasks': 'Tasks',
  'tab.notes': 'Shopping',
  'tab.profile': 'Finance',

  // Today
  'today.greeting.morning': 'Good Morning',
  'today.greeting.afternoon': 'Good Afternoon',
  'today.greeting.evening': 'Good Evening',
  // Opening moment
  'intro.l1': 'Create Your',
  'intro.accent': 'Tasks',
  'intro.l2': 'And Manage',
  'intro.l3': 'Your Work',
  'intro.start': 'Start',

  'today.headline.line1': 'Plan your day,',
  'today.headline.line2': 'get things done',
  'today.progress.title': 'Today’s Progress',
  'today.progress.total': 'Total Task',
  'today.progress.completed': 'Completed Task',
  'today.progress.pending': 'Pending Task',
  'today.tasks.title': 'Today’s Tasks',
  'today.tasks.viewAll': 'View All',
  'today.ai.label': 'Today’s AI Analysis',
  'today.ai.report': 'AI-Report',
  'today.ai.prefix': 'You Have {{count}} Tasks',
  'today.ai.suffix': 'for Today.',
  'today.ai.urgent': 'Urgent',
  'today.priorityTask': 'Priority Task',
  'today.priorityEmpty': 'Nothing on the list yet.',
  'today.completed': 'Completed',
  'today.completedCount': '{{done}}/{{total}} task',
  'today.inProgress': 'In Progress',
  'today.taskCount': '{{count}} task',
  'today.allTasks': 'All Tasks',
  'today.dueDate': 'Due Date: {{date}}',
  'today.type.event': 'Meeting',
  'today.type.task': 'Task',
  'today.filter.todo': 'To Do',
  'today.filter.inprogress': 'In Progress',
  'today.filter.done': 'Done',
  'today.filter.left': 'Left',
  'today.explore': 'Here’s your day',
  'today.overview': 'Overview',
  'today.yourTasks': 'Your Tasks',
  'today.completedOf': '{{done}}/{{total}} completed',
  'today.status.done': 'Completed',
  'today.status.inprogress': 'Happening now',
  'today.status.overdue': 'Overdue',
  'today.status.scheduled': 'Scheduled',
  'today.empty.title': 'No tasks here',
  'today.empty.body': 'Tap the + button to add your first task for today.',
  'today.setName': 'Set your name in Profile',
  'today.priority': '{{level}} Priority',
  'today.allDay': 'All day',
  'today.hello': 'Hello, {{name}}!',
  'today.friend': 'there',
  'today.tile.tasks': 'Tasks',
  'today.tile.events': 'Events',
  'today.tile.today': 'Today',
  'today.tile.productivity': 'Productivity',
  'today.upcomingEvent': 'Upcoming Event',
  'today.noEvents': 'Nothing scheduled',
  'today.addNewTask': 'Add New Task',
  'today.completedDone': '{{count}} completed',
  'calendar.view.day': 'Day',
  'calendar.view.week': 'Week',
  'calendar.view.month': 'Month',
  'tasks.title': 'My Tasks',
  'tasks.filter.upcoming': 'Upcoming',
  'tasks.completed': 'Completed',
  'tasks.countTasks': '{{count}} tasks',

  // Calendar
  'calendar.headline.line1': 'Task',
  'calendar.headline.line2': 'Schedule',
  'calendar.button': 'Calendar',
  'calendar.filter.all': 'All',
  'calendar.filter.work': 'Work',
  'calendar.filter.personal': 'Personal',
  'calendar.section.allDay': 'All Day',
  'calendar.section.morning': 'Morning',
  'calendar.section.afternoon': 'Afternoon',
  'calendar.section.evening': 'Evening',
  'calendar.status.inprogress': 'In Progress',
  'calendar.status.todo': 'Upcoming',
  'calendar.status.done': 'Done',
  'calendar.today': 'Today',

  // Tasks
  'tasks.headline.line1': 'My',
  'tasks.headline.line2': 'Tasks',
  'tasks.filter.all': 'All',
  'tasks.filter.open': 'Open',
  'tasks.filter.done': 'Done',
  'tasks.empty.title': 'No tasks yet',
  'tasks.empty.body': 'Tap + to add your first task.',
  'tasks.error.title': 'Couldn’t load tasks',
  'tasks.noDue': 'No due date',
  'tasks.tomorrow': 'Tomorrow',

  // Weather
  'sky.clear': 'Clear',
  'sky.partly': 'Partly cloudy',
  'sky.cloud': 'Cloudy',
  'sky.fog': 'Foggy',
  'sky.drizzle': 'Light drizzle',
  'sky.rain': 'Rain',
  'sky.snow': 'Snow',
  'sky.storm': 'Thunderstorms',
  'weather.feelsLike': 'Feels like {{deg}}°',
  'weather.range': 'H {{high}}°  L {{low}}°',

  // Home — now / next
  'home.now': 'Now',
  'home.next': 'Next',
  'home.tasksToday': '{{count}} tasks today',
  'home.upNext': 'Up next',
  'home.nothingNow': 'Nothing on right now',
  'home.nothingNext': 'Nothing after this',
  'home.noMeetings': 'No meetings today',
  'home.done': 'Done',
  'home.undo': 'Undo',
  'home.snooze': 'Tomorrow',
  'home.open': 'Open',
  'home.allDone': 'Everything is done',
  'home.allDoneBody': 'Nothing left on today’s list.',
  // The four squares: one per part of the app, opened from home.
  'home.sections': 'Your four',
  'home.section.tasks': 'Tasks',
  'home.section.open': '{{count}} open',
  'home.section.calendar': 'Calendar',
  'home.section.events': '{{count}} today',
  'home.section.shopping': 'Shopping',
  'home.section.finance': 'Finance',

  // Country & currency
  'onboarding.place.title': 'Where are you?',
  'onboarding.place.body': 'It sets the currency your money is shown in.',
  'onboarding.place.currency': 'Currency',
  'country.IL': 'Israel',
  'country.US': 'United States',
  'country.GB': 'United Kingdom',
  'country.DE': 'Germany',
  'country.FR': 'France',
  'country.ES': 'Spain',
  'country.IT': 'Italy',
  'country.NL': 'Netherlands',
  'country.CA': 'Canada',
  'country.AU': 'Australia',
  'country.CH': 'Switzerland',
  'country.RU': 'Russia',
  'country.UA': 'Ukraine',
  'country.TR': 'Türkiye',
  'country.AE': 'UAE',
  'country.IN': 'India',
  'country.BR': 'Brazil',
  'country.JP': 'Japan',
  'country.other': 'Somewhere else',

  // Shopping list
  'shop.title': 'Shopping List',
  'shop.add.placeholder': 'Add an item…',
  'shop.add': 'Add item',
  'shop.toBuy': 'To buy',
  'shop.bought': 'In the trolley',
  'shop.clearBought': 'Clear bought',
  'shop.clearBoughtConfirm': 'Remove the {{count}} items already in the trolley?',
  'shop.empty.title': 'The list is empty',
  'shop.empty.body': 'Type an item above, or ask her to add one for you.',
  'shop.edit': 'Edit item',
  'shop.name': 'Item',
  'shop.quantity': 'Quantity',
  'shop.quantityHint': 'e.g. 2, 500g, a bunch',
  'shop.note': 'Note',
  'shop.category': 'Category',
  'shop.noCategory': 'No category',
  'shop.cat.produce': 'Fruit & veg',
  'shop.cat.dairy': 'Dairy',
  'shop.cat.meat': 'Meat & fish',
  'shop.cat.bakery': 'Bakery',
  'shop.cat.cleaning': 'Cleaning',
  'shop.cat.pharmacy': 'Pharmacy',
  'shop.cat.other': 'Other',

  // Finance
  'fin.title': 'Finance',
  'fin.income': 'Income',
  'fin.expenses': 'Expenses',
  'fin.balance': 'Balance',
  'fin.debts': 'Debts',
  'fin.addIncome': 'Add income',
  'fin.addExpense': 'Add expense',
  'fin.addDebt': 'Add debt',
  'fin.editIncome': 'Edit income',
  'fin.editExpense': 'Edit expense',
  'fin.editDebt': 'Edit debt',
  'fin.description': 'Description',
  'fin.amount': 'Amount',
  'fin.date': 'Date',
  'fin.category': 'Category',
  'fin.person': 'Name',
  'fin.personHint': 'Who the debt is with',
  'fin.owe': 'I owe',
  'fin.owed': 'Owed to me',
  'fin.markPaid': 'Mark as paid',
  'fin.markUnpaid': 'Reopen',
  'fin.paid': 'Paid',
  'fin.open': 'Open',
  'fin.settled': 'Settled',
  'fin.empty.income': 'No income recorded yet.',
  'fin.empty.expenses': 'No expenses recorded yet.',
  'fin.empty.debts': 'Nothing owed either way.',
  'fin.missingAmount': 'Enter an amount',
  'fin.missingAmountBody': 'An amount greater than zero is needed.',
  'fin.missingDescription': 'Enter a description',
  'fin.missingDescriptionBody': 'Say what this was for.',
  'fin.inc.salary': 'Salary',
  'fin.inc.business': 'Business',
  'fin.inc.refund': 'Refund',
  'fin.inc.gift': 'Gift',
  'fin.inc.other': 'Other',
  'fin.exp.shopping': 'Shopping',
  'fin.exp.food': 'Food',
  'fin.exp.housing': 'Housing',
  'fin.exp.bills': 'Bills',
  'fin.exp.transport': 'Transport',
  'fin.exp.health': 'Health',
  'fin.exp.fun': 'Leisure',
  'fin.exp.other': 'Other',

  // Notes
  'notes.title': 'My Notes',
  'notes.empty.title': 'No notes yet',
  'notes.empty.body': 'Tap + and tell her what to remember.',
  'notes.close': 'Close',

  // Add / edit
  'form.newTask': 'Add New Task',
  'form.newEvent': 'Add New Event',
  'form.taskTitle': 'Task Title',
  'form.eventTitle': 'Event Title',
  'form.taskPlaceholder': 'Finish landing page design',
  'form.eventPlaceholder': 'Meeting with client',
  'form.description': 'Description',
  'form.descriptionPlaceholder': 'Design the new landing page for the product launch.',
  'form.startDate': 'Start Date & time',
  'form.dueDate': 'Due Date & time',
  'form.priority': 'Priority',
  'form.priority.low': 'Low',
  'form.priority.medium': 'Medium',
  'form.priority.high': 'High',
  'form.createTask': 'Create Task',
  'form.createEvent': 'Create Event',
  'form.missingTitle': 'Missing title',
  'form.missingTitleBody': 'Please enter a title.',
  'form.missingDate': 'Missing date',
  'form.missingDateBody': 'Enter a date as YYYY-MM-DD.',
  'form.saveFailed': 'Couldn’t save',

  // Assistant
  'assistant.title': 'Assistant',
  'assistant.subtitle': 'Ask for a task or an event',
  'assistant.trySaying': 'Try saying',
  'assistant.example1': 'Remind me to call the dentist tomorrow at 10',
  'assistant.example2': 'Meeting with Maya on Thursday at 2pm',
  'assistant.example3': 'Add a task to write the release notes',
  'assistant.placeholder': 'Type a request…',
  'assistant.listening': 'Listening…',
  'assistant.thinking': 'Thinking…',
  'assistant.transcribing': 'Transcribing…',
  'assistant.confirm': 'Confirm',
  'assistant.notNow': 'Not now',
  'assistant.addEvent': 'Add this event?',
  'assistant.addTask': 'Add this task?',
  'assistant.done': 'Done',
  'assistant.recordHint': 'Tap the square when you’re done',
  'assistant.failed': 'Something went wrong',

  // Voice assistant
  'voice.title': 'Your assistant',
  'voice.starting': 'One moment…',
  'voice.listening': 'Listening',
  'voice.thinking': 'Thinking…',
  'voice.speaking': 'Speaking',
  'voice.tapToStop': 'Tap when you’re done',
  'voice.tapToInterrupt': 'Tap to cut in',
  'voice.hint': 'Just talk — she’ll do the rest',
  'voice.ended': 'We’re done for now',
  'voice.restart': 'Talk again',
  'voice.hiccup': 'Sorry — I lost that one. Say it again?',
  'voice.noCredits':
    'My account has run out of credits, so I can’t hear or answer right now. Top it up and I’ll be right back.',
  'voice.notConfigured':
    'I’m not set up on the server yet — my API key is missing or rejected. Once that’s fixed I’ll be right back.',
  'voice.drawing': 'Drawing…',
  'voice.offerDone': 'Done — {{time}}.',
  'voice.offerPicked': "Let's make it {{time}}.",
  'voice.offerFailed': "I couldn't put that in the diary.",
  'voice.drawFailed': "I couldn't draw that one.",
  'voice.newChat': 'Start a new conversation',
  'voice.undo': 'Undo that',
  'voice.micDenied': 'She needs the microphone to hear you.',
  'voice.unavailable': 'This build has no microphone support — run a development build to talk to her.',

  // Profile
  'profile.headline.line1': 'Your',
  'profile.headline.line2': 'Profile',
  'profile.workspace': 'Personal workspace',
  'profile.onDevice': 'On this device',
  'profile.completion': 'Task completion',
  'profile.stat.tasks': 'Tasks',
  'profile.stat.done': 'Done',
  'profile.stat.streak': 'Day streak',
  'profile.stat.events': 'Events',
  'profile.openTasks': 'Open tasks',
  'profile.completed': 'Completed',
  'profile.displayName': 'Display Name',
  'profile.yourName': 'Your name',
  'profile.notifications': 'Notifications',
  'profile.notificationsBody': 'Reminders for upcoming tasks',
  'profile.notificationsBlocked': 'Notifications are turned off',
  'profile.notificationsBlockedBody':
    'This app is not allowed to notify you, so reminders will not arrive. You can turn them on in the system settings.',
  'profile.openSettings': 'Open settings',
  'profile.week': 'Your week',
  'profile.weekBody': 'What she plans around. Change these whenever they change.',
  'profile.language': 'Language',
  'profile.languageBody': 'Changes the app’s text and layout direction',
  'profile.storage': 'Storage',
  'profile.storageBody': 'Everything is kept on this device. Nothing is uploaded.',
  'profile.clearData': 'Clear all data',
  'profile.clearConfirm': 'Clear all data',
  'profile.clearConfirmBody':
    'Deletes every task and event on this device. This cannot be undone.',
  'profile.deleteEverything': 'Delete everything',
  'profile.save': 'Save Settings',
  'profile.saved': 'Saved',
  'profile.savedBody': 'Your settings were updated.',
  'profile.restartTitle': 'Restart needed',
  'profile.restartBody':
    'The layout direction changed. Close and reopen the app to apply it.',

  // Shared
  'today.planned': 'Planned for today',
  'assistant.send': 'Send',
  'form.editTask': 'Edit Task',
  'form.editEvent': 'Edit Event',
  'form.endTime': 'End time',
  'form.badEndTime': 'Check the times',
  'form.badEndTimeBody': 'The end time has to be after the start time.',
  'notify.taskTitle': 'Task due now',
  'notify.eventTitle': 'In {{minutes}} minutes',
  'profile.legal': 'About & legal',
  'profile.legalBody': 'Policies, support and erasing your data',
  'profile.privacy': 'Privacy policy',
  'profile.terms': 'Terms of use',
  'profile.contact': 'Contact us',
  'profile.contactSubject': 'Personal Assistant — support',
  'profile.linkFailed': 'Couldn’t open that link',
  'profile.deleteAccount': 'Delete account and data',
  'profile.deleteAccountBody': 'Erases every task, event and conversation on this device, and resets your settings. This cannot be undone.',
  'profile.deleted': 'Everything deleted',
  'profile.deletedBody': 'Nothing is left on this device.',
  'common.close': 'Close',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.error': 'Error',
  'common.deleteTitle': 'Delete',
  'common.deleteBody': 'Delete “{{title}}”?',

  // Onboarding
  // The questionnaire on first launch. Language is asked first, and every
  // screen after it is already in the language that was chosen.
  'onboarding.step': '{{current}} of {{total}}',
  'onboarding.next': 'Continue',
  'onboarding.back': 'Back',
  'onboarding.skip': 'Skip for now',
  'onboarding.finish': 'Start using the app',
  'onboarding.intro.title': 'This is how it works',
  'onboarding.intro.body':
    'Three things worth knowing before we start. The rest takes about a minute.',
  'onboarding.intro.talk.title': 'Talk to her',
  'onboarding.intro.talk.body':
    'Say what is happening, the way you would say it to a person. No forms unless you want them.',
  'onboarding.intro.plan.title': 'She keeps the diary',
  'onboarding.intro.plan.body':
    'Tasks and meetings land in the right place, at a time that actually works around the rest of your day.',
  'onboarding.intro.private.title': 'It stays on your phone',
  'onboarding.intro.private.body':
    'Your diary is stored on this device. There is no account and nothing to sign into.',

  'onboarding.permissions.title': 'Two things she needs',
  'onboarding.permissions.body':
    'Both are optional, and you can change your mind later in Settings.',
  'onboarding.permissions.allow': 'Allow',
  'onboarding.permissions.allowed': 'Allowed',
  'onboarding.permissions.optional':
    'Skip either one and the app still works — you can type to her instead of talking, and everything is saved either way.',
  'onboarding.permissions.mic.title': 'Microphone',
  'onboarding.permissions.mic.body': 'So you can talk to her instead of typing.',
  'onboarding.permissions.notify.title': 'Reminders',
  'onboarding.permissions.notify.body': 'So she can tell you before something starts.',

  'onboarding.language.title': 'Choose your language',
  'onboarding.language.body':
    'The app and your assistant will both speak it. You can change this later.',
  'onboarding.name.title': 'What should she call you?',
  'onboarding.name.body': 'Just a first name is plenty.',
  'onboarding.work.title': 'When do you work?',
  'onboarding.work.body': 'She keeps meetings inside these hours unless you say otherwise.',
  'onboarding.sleep.title': 'When do you sleep?',
  'onboarding.sleep.body': 'Nothing will ever be scheduled in here.',
  'onboarding.sleep.genderPrompt': 'Quick one, so we can tailor the recommendation:',
  'onboarding.sleep.gender.male': 'Man',
  'onboarding.sleep.gender.female': 'Woman',
  'onboarding.sleep.gender.skip': "I'd rather not say",
  'onboarding.sleep.recommended.male': 'Recommended for you: 7–9 hours a night.',
  'onboarding.sleep.recommended.female':
    'Recommended for you: 7–9 hours a night — many women feel best nearer the top of that range.',
  'onboarding.sleep.recommended.unspecified': 'Recommended: 7–9 hours a night for most adults.',
  'onboarding.sleep.fit.good': '{{hours}}h a night — right on target.',
  'onboarding.sleep.fit.short': '{{hours}}h a night — a little short of that.',
  'onboarding.sleep.fit.long': '{{hours}}h a night — comfortably more than that.',
  'onboarding.buffer.title': 'How much room between meetings?',
  'onboarding.buffer.body': 'Time to travel, write things up, or breathe.',
  'onboarding.buffer.none': 'Back to back',
  'onboarding.buffer.minutes': '{{count}} min',
  'onboarding.types.title': 'What fills your days?',
  'onboarding.types.body': 'Pick as many as fit. It helps her sort what you tell her.',
  'onboarding.type.work': 'Work',
  'onboarding.type.study': 'Study',
  'onboarding.type.family': 'Family',
  'onboarding.type.health': 'Health',
  'onboarding.type.sport': 'Exercise',
  'onboarding.type.social': 'Friends',
  'onboarding.type.errands': 'Errands',
  'onboarding.fixed.title': 'Anything that never moves?',
  'onboarding.fixed.body': 'Standing commitments she should plan around.',
  'onboarding.fixed.placeholder': 'Gym Mon & Wed at 6am, family dinner Friday evening',
  'onboarding.done.title': 'That is everything',
  'onboarding.done.body': 'She knows how your week is shaped. Talk to her whenever you like.',
  'onboarding.hour': '{{hour}}:00',
};
